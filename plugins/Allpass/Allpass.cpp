...
protected:
  // metadata
  const char *getLabel() const override { return "ObtuseAllpass"; }
  ...
  void setParameterValue(uint32_t index, float value) override {
    ...
    case kDecay:
      decay = value;
      effectsXL_Allpass_setDecay(context_processor, float_to_fix(decay));
      break;
    case kDelay:
      delay = value;
      updateDelay();
  ...
  void run(const float** inputs, float** outputs, uint32_t frames) override {
    ...
    // process and mix
      ...
      while (k < frames) {
        ...
        // run DSP
        effectsXL_Allpass_process_bufferTo(context_processor, chunkSize, buffIn, buffOut);
...
private:
  effectsXL_Allpass_process_type context_processor;
  ...
  void updateDelay() {
      ...
      effectsXL_Allpass_setDelayms(context_processor, float_to_fix(realDelay));
  }

  DISTRHO_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(Allpass);
};

Plugin *createPlugin() { return new Allpass(); }
